...
int computate_equation(const QVector<long double> x, const QVector<long double> y, long double& slope, long double& intercept)
{
    ...
    if(x.size() < 2){
        throw InvalidComputation(QObject::tr("ERROR: less than two data points found"));
    }
    if(!(x.size() * sumx2 - sumx * sumx)){
        slope = 0;
        intercept = sumx/(double)x.size();
        throw ComputationWarning(QObject::tr("slope is equal to zero. Data points form a vertical line."));
    }
